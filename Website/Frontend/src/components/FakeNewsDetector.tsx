...
export function FakeNewsDetector() {
  ...
  const analyzeWebsite = async () => {
   ...
   try {
    ...
    setResult({
      status: data.ui_metrics.status === "SAFE" ? "safe" : "fake",
      score: data.ui_metrics.confidence_score,
      factors: [
        {
          name: "Prediction",
          status: data.ui_metrics.status === "SAFE" ? "pass" : "fail",
          description: data.ui_metrics.prediction,
        },
        {
          name: "Risk Score",
          status: "warning",
          description: `${data.ui_metrics.risk_score}%`,
        },
        {
          name: "Recommendation",
          status: "warning",
          description: data.ui_metrics.recommendation,
        },
        ],  
      });
    } finally {
